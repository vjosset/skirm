...
|DAM|**Damage**|The amount of damage this weapon inflicts on its targets for each successful strike.|
...
1. **Roll Saves** - Determine how much damage the Attacker inflicted on the Target
    ...
    1. If a die result is equal to or lower than `ARM` - `DAM`, then the Target's Armor absorbed the damage from this strike
...
1. **Roll Strikes** - Determine successful strikes
...
1. **Compare Strikes** - Determine which strikes made it through
    1. For each successful strike that either Agent got above the other Agent, the Agent with fewer successful strikes rolls one save
    1. The Agent with more strikes is the "winner"; the Agent with fewer strikes is the "loser"
    1. Reduce the number of the winner's successful strikes by the loser's number of successful strikes - These are strikes that the loser could not counter or parry
1. **Roll Saves** - Determine how much damage the loser received
    1. For each of the winner's remaining successful strikes, the loser rolls one Save die
