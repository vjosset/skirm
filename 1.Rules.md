...
A die roll of `1` is a **Critical** success and is always a success, treated as two successes. Some rules may change the die results that are treated as **Critical** (for example `CR2` indicates that die rolls of `1` and `2` are treated as Critical, and therefore count as two successes each).
...
At the start of each turn, each player rolls `3D6` and adds 1 die for each Agent's `LDx` Special Rule. For example, if your Leader is still in play and has the `LD2` Special Rule, you would roll `5D6` for Tactical Orders (3 base dice for Tactical Orders plus 2 dice provided by your Leader).  
...
1. **Roll Hits** - Determine if the Attacker's weapon reached the Target
    ...
    1. Each die result that is equal to or lower than the selected weapon's `SKL` (Skill) is a successful Hit. Note that die results of 1 are always successes and count as two successes, and die results of 6 are always failures.
...
|`HLx`|**Heal x**|This Agent regains `x` lost `HIT` at the end of every Turn|
