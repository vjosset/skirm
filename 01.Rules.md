...
|`CTR`|**Counter**|Each time this Unit is the target of a Melee Attack, if this Unit is Standing all Attacks fail, it may immediatey perform a Melee Attack against the original Attacker.|
|`DOD`|**Dodge**|Each time this Unit is the target of a Melee Combat Attack, if it is Standing, it gets one automatic successful save (without having to roll for it). Note that this automatic save does not save against `APx` (Armor Piercing) successful strikes.|
|`LDx`|**Leader x**|While this Unit is Standing, you can roll for an additional `x` Tactical Orders every turn.|
|`MX1`|Max 1|A Squad may only have one of this Unit.|
|`PAR`|**Parry**|Each time this Unit is the target of a Melee Attack, if it is Standing, it may choose one of the Attacker's Attack Dice to be re-rolled.|
...
|`MX1`|Max 1|A Squad may only have one of this Weapon.|
...
|`THR`|**Thrown**|Weapon is a Ranged Weapon but the Unit may use its `MSK`` or `RSK` Stat to determine successful strikes.|
