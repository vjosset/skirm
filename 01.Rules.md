...
During its [activation](#alternate-activations), an Unit can perform a number of Actions up to its `ACT` stat *(see [StatCards](#stat-cards))*.  
...
If a player has fewer Units in play than their opponent, you may run into cases where your opponent still has Units to activate but all your own Units have already been activated. In those cases, when it is your turn to activate an Unit, you may choose to have one of your Units perform a [Sentry action](#sentry-actions).  
...
Skills that have an AP cost can be performed using the Unit's AP Stat **or** a [Tactical Order](#tactical-orders).
...
**Recover (1 ACT)**
The Downed Unit attempts to recover. The Unit performs a Recovery Roll by rolling one die against its `ARM` stat. Die results equal to or lower than the Unit's `ARM` stat are successful recoveries, and the Unit goes from Downed to Standing, and can now perform other Actions.  
This is the only Action a Downed Unit may perform.
...
When a Standing Unit fails Save rolls equal to or higher than its `HIT` Stat, it goes to a Downed State.
When a Downed Unit fails Save rolls equal to or higher than its `HIT` Stat, it is Out and removed from the battlemap.  
Downed Units cannot perform any Action other than Recover during their Activation.
