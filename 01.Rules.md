...
|`CAM`|**Camouflage**|Each time this Unit is the target of a Ranged Combat Attack, if it is Standing, it gets one automatic successful save (without having to roll for it). Note that this automatic save does not save against `APx` (Armor Piercing) successful strikes.|
