...
|`CAM`|**Camouflage**|Each time this Unit is the target of a Ranged Combat Attack, if iot is Standing, it gets one automatic successful save (without having to roll for it). Note that this automatic save does not save against `APx` (Armor Piercing) successful strikes.|
...
|`RSM`|**Reroll Saves - Melee**|Each time this Unit is the target of a Melee attack, it may re-roll any number of its Save rolls.|
|`RSR`|**Reroll Saves - Ranged**|Each time this Unit is the target of a Ranged attack, it may re-roll any number of its Save rolls.|
...
|`THR`|**Thrown**|Weapon is a Ranged Weapon but the Unit may choose to use either its `MSK` or `RSK` Stat to determine successful strikes.|
