...
1. **Mission Start**
   1. **Select The [Mission](03.Missions)**  
...
|Mods|**Special**|Special rules for this Unit (see [Unit Mods](#Unit-mods)).|
...
|Mods|**Special**|Special rules for this weapon (see [Weapon Mods](#weapon-mods)).|
...
When an Unit moves on the Battlemap, the maximum distance it can travel is given in Squares by its `MOV` stat. Movement can be done along any cardinal direction (North, South, East, West) or in diagonal (North-East, North-West, South-East, South-West).
...
When a Standing Unit fails Save rolls equal to or higher than its `HIT` Stat, it goes to a Downed State. Downed Units are placed on their side in their current Square to indicate their State.  
...
See [Factions](02.Factions) for Squads, Units, Weapons, Gear, and Mods.
...
|`RAM`|**Reroll Attacks - Melee**|This Unit may re-roll any Melee Combat attack dice.|
|`RAR`|**Reroll Attacks - Ranged**|This Unit may re-roll any Ranged Combat attack dice.|
